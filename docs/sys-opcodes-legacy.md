...
> Historical reference only. This document describes the old immediate 8-bit SYS/HAL mapping and is not the current engine contract. See [current SYS semantics](sys-opcodes.md). The examples below must not be used with the current engine without migration.
